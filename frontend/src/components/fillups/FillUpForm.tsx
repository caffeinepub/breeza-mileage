...
export default function FillUpForm() {
  ...
  const [errors, setErrors] = useState<{ odometer?: string; fuelAdded?: string; totalFuelCost?: string }>({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validation = validateFillUp(odometer, fuelAdded, totalFuelCost, lastOdometer);
    ...
    try {
      ...
      const costValue = totalFuelCost.trim() ? parseFloat(totalFuelCost) : 0;
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              className={errors.totalFuelCost ? 'border-destructive' : 'bg-background'}
              disabled={!canSubmit}
            />
            {errors.totalFuelCost && (
              <p className="text-sm text-destructive">{errors.totalFuelCost}</p>
            )}
          </div>
